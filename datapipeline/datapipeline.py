...
from powerdatapipeline.datapipeline.tfdataset import df_to_dictdataset,csv_to_csvdataset,tfdataset_to_windowed_tfdataset,tfdataset_to_batched_tfdataset,csvdataset_to_tfdataset
from powerdatapipeline.datapipeline.tfdataset import tdataset_to_timefeatures_dataset,concatenate_dataset_features,add_normalizer_to_tfdataset,zip_datasets
from powerdatapipeline.datapipeline.tfdataset_utilities import compare_tfdataset_elements,show_tfdataset_element,tfdataset_to_numpyarray,get_normalizer_from_tfdataset,show_tfdataset_cardinatlity,check_normalizer
from powerdatapipeline.datapipeline.tfdataset_utilities import add_columns_to_csvdataset,get_tfdataset_element,check_equality_in_dataset,get_interval_dataset
...
def get_dictdataset_from_csv(config_dict:dict,csv_folder:str,csv_file:str,n_rows:int):
	...
	columns_original = config_dict["data_pipeline"]["extraction"]["columns_original"] #["index","time", "va", "vb", "vc", "ia", "ib", "ic", "Vdc", "Idc"]
	...
	if use_df:
		...
		df = get_df_der(data_folder = csv_folder,csv_file=csv_file,n_rows=n_rows,use_existing_columnnames=use_existing_columnnames,
				        columns_original=columns_original,columns_added=columns_added,datetime_column=column_datetime,
				        show_df=show_df) #der_data_sample	
				
		csvdataset = df_to_dictdataset(df,columns_selected)
		csv_columns = columns_selected
		#print(f"Dataset:{csvdataset}")
	else:
		...
		csvdataset,n_rows_in_csvdataset = csv_to_csvdataset(csv_filepattern=os.path.join(csv_folder,csv_file),
							           columns_original=columns_original,columns_selected=columns_selected,use_existing_columnnames=use_existing_columnnames,
									   n_rows=n_rows)
...
def filter_out_string(csvdataset:tf.data.Dataset,selected_column:str,filtered_string:str) -> tf.data.Dataset:
	...
	csvdataset = csvdataset.filter(lambda x: keras.ops.equal(x[selected_column][0],filtered_string))  #tf.math.equal(x[selected_column][0],filtered_string)
...
def get_dataset_from_rawdataset(rawdataset:tf.data.Dataset,dataset_columns:List[str],dataset_name:str,
	...
	if features_numerical:
		dataset_numerical,features_numerical_updated = get_dataset_numerical(rawdataset,dataset_columns,features_numerical,dataset_name)
	...
	if features_onehot: #One hot features are categorical features that must be converted to one hot features
		dataset_onehot_dict,features_onehot_expanded = get_dataset_onehot(rawdataset,dataset_columns,features_onehot,dataset_name)
		dataset_dict.update(dataset_onehot_dict)
	else:
		features_onehot_expanded = []
	
	if features_time:		
		dataset_time,features_time_updated = get_dataset_time(rawdataset,dataset_columns,features_time,feature_datetime,dataset_name,plot_datasets)
		dataset_dict.update({"time_features":dataset_time})
	else:
		features_time_updated = []
			
	for dataset_key,dataset1 in dataset_dict.items():
		if isinstance(dataset1.element_spec, collections.OrderedDict) or isinstance(dataset1.element_spec, dict): #For csv dataset
	...
	print(f"Building dataset with features:{features_numerical_updated+features_onehot_expanded+features_time_updated}")	
...
def get_normalized_dataset(dataset:tf.data.Dataset,normalizer,dataset_name:str,n_elements_to_plot:int=100,plot_datasets:bin=False) -> tf.data.Dataset:
	dataset = add_normalizer_to_tfdataset(dataset,normalizer)
	show_tfdataset_element(dataset,f"{dataset_name}_dataset_normalizer")
	
	dataset_array_normalized = tfdataset_to_numpyarray(dataset,n_elements_to_plot,dataset_name=f"{dataset_name}_features_normalized")
...
def get_transformed_dataset(dataset:tf.data.Dataset,window_size:int,batch_size:int,dataset_name:str) -> tf.data.Dataset:
	if window_size > 1:
	...
	show_tfdataset_element(dataset,f"{dataset_name}_dataset_windowed_batched",show_first_element_only=True) #Only show first element of batched dataset to reduce logs

	return dataset

def get_dataset_numerical(dataset:tf.data.Dataset,dataset_columns:str,numerical_features:List[str],dataset_name:str):
	...
	if isinstance(dataset.element_spec, collections.OrderedDict) or isinstance(dataset.element_spec, dict):
		...
		numerical_feature_indexes = [dataset_columns.index(numerical_feature) for numerical_feature in numerical_features]
		print(f"Numerical {dataset_name} feature indexes:{numerical_feature_indexes}")		
		dataset_numerical = dataset.map(lambda window: tf.gather(window, numerical_feature_indexes, axis=-1)) #Gather indices for input numberical features
	
	return dataset_numerical,numerical_features

def get_dataset_time(dataset:tf.data.Dataset,dataset_columns:str,time_features:List[str],datetime_feature_column:str,dataset_name:str,plot_datasets=False,n_elements_to_plot=10):
	
	datetime_feature_index = dataset_columns.index(datetime_feature_column) #There can only be one datetime feature index
	print(f"Datetime index:{datetime_feature_index}")
	
	print(f"Adding time features:{time_features}")	
	time_features_cyclical = [f"{cyclical}{time_feature}" for time_feature in time_features for cyclical in ["sin","cos"]]
	dataset_time = tdataset_to_timefeatures_dataset(dataset,datetime_index=datetime_feature_index,time_features=time_features)

	if plot_datasets:
		time_features_array = tfdataset_to_numpyarray(dataset_time,n_elements_to_plot,dataset_name=f"{dataset_name}_time_features")
		plot_cyclical_time_features(time_features_array,time_features_cyclical,f"plots/tfdataset_{dataset_name}_time_features.png")
...
def get_dataset_onehot(dataset,dataset_columns,onehot_features,dataset_name):
	...
	for onehot_feature in onehot_features:
		if isinstance(dataset.element_spec, collections.OrderedDict) or isinstance(dataset.element_spec, dict):
			dataset_onehot = dataset.map(lambda example: {f"{onehot_feature}_onehot": tf.one_hot(example[onehot_feature], depth=n_categories)})
			dataset_onehot = dataset_onehot.flat_map(lambda x: tf.data.Dataset.from_tensor_slices(x)) #Flattens the nested datasets into a single dataset					
		else:
			onehot_feature_index = dataset_columns.index(onehot_feature)
			print(f"One hot {dataset_name} feature index:{onehot_feature_index}")
			dataset_onehot = dataset.map(lambda window: tf.gather(window, [onehot_feature_index], axis=-1)) #Gather indices for input categorical features
			dataset_onehot =  keras.layers.CategoryEncoding(num_tokens=n_categories, output_mode="one_hot")(dataset_onehot) #Convert categorical to one hot
			
		onehot_features_expanded.extend([f"{onehot_feature}_onehot_{onehotindex}" for onehotindex in range(n_categories)])
...
def get_dataset_with_selected_features(dataset:tf.data.Dataset,features:List[str]) -> tf.data.Dataset:
	...
	assert isinstance(dataset.element_spec, collections.OrderedDict) or isinstance(dataset.element_spec, dict), f"{dataset.element_spec} is not valid!"
	...
	for feature_spec in feature_specs:
		...
		if feature_spec["feature_type"] == "numerical":
			for column_name in feature_spec["features"]:
				print(f"Creating numerical (float) feature space for {column_name} with output mode:{feature_spec['output_mode']}")
				if feature_spec["output_mode"] == "plain":
					feature_space_definitions.update({column_name:keras.utils.FeatureSpace.float()}) # Numerical features not normalized
				elif feature_spec["output_mode"] == "normalized":
					feature_space_definitions.update({column_name:keras.utils.FeatureSpace.float_normalized()}) # Numerical features normalized
				elif feature_spec["output_mode"] == "rescaled":
					feature_space_definitions.update({column_name:keras.utils.FeatureSpace.float_rescaled()}) # Numerical features linearly rescaled
				else:
					raise ValueError(f"{feature_spec['output_mode']} is not valid!")

		elif feature_spec["feature_type"] == "int":
			for column_name in feature_spec["features"]:
				print(f"Creating categorical feature space for {column_name} with output mode:{feature_spec['output_mode']}")
				if feature_spec["output_mode"] == "int":				
					feature_space_definitions.update({column_name:keras.utils.FeatureSpace.integer_categorical(output_mode="int")}) # Categorical feature encoded as string				
				if feature_spec["output_mode"] == "one_hot":
					feature_space_definitions.update({column_name:keras.utils.FeatureSpace.integer_categorical(num_oov_indices=1,output_mode="one_hot")}) # Categorical feature encoded as string				
				else:
					raise ValueError(f"{feature_spec['output_mode']} is not valid!")

		elif feature_spec["feature_type"] == "string":
			for column_name in feature_spec["features"]:
				print(f"Creating string categorical feature space for {column_name} with output mode:{feature_spec['output_mode']}")
				if feature_spec["output_mode"] == "int":
					feature_space_definitions.update({column_name:keras.utils.FeatureSpace.string_categorical(output_mode="int")}) # Categorical feature encoded as string
				elif feature_spec["output_mode"] == "one_hot":
					feature_space_definitions.update({column_name:keras.utils.FeatureSpace.string_categorical(num_oov_indices=0,output_mode="one_hot")}) # Categorical feature encoded as string
				else:
					raise ValueError(f"{feature_spec['output_mode']} is not valid!")
		
		elif feature_spec["feature_type"] == "datetimestamp_seconds":
			for column_name in feature_spec["features"]:
				print(f"Creating cyclical feature space for {column_name} with output mode:{feature_spec['output_mode']}")
				if feature_spec["output_mode"] == "plain":
					feature_space_definitions.update({column_name:keras.utils.FeatureSpace.float()}) # Numerical time feature not normalized					
				elif feature_spec["output_mode"] == "cyclical_minute_hour_day":
					feature_space_definitions.update({column_name:keras.utils.FeatureSpace.feature(preprocessor=keras.layers.Lambda(cyclical_minute_hour_day,dtype=tf.float64), dtype=tf.float64,output_mode="float"),
														}) # minutes,hours, and days in datetime are encoded using cyclical features
				elif feature_spec["output_mode"] == "cyclical_minute":
					feature_space_definitions.update({column_name:keras.utils.FeatureSpace.feature(preprocessor=keras.layers.Lambda(cyclical_minute,dtype=tf.float64), dtype=tf.float64,output_mode="float"),                                                 
													}) # minutes,hours, and days in datetime are encoded using cyclical features
				elif feature_spec["output_mode"] == "cyclical_hour":
					feature_space_definitions.update({column_name:keras.utils.FeatureSpace.feature(preprocessor=keras.layers.Lambda(cyclical_hour,dtype=tf.float64), dtype=tf.float64,output_mode="float"),
													}) # minutes,hours, and days in datetime are encoded using cyclical features
				elif feature_spec["output_mode"] == "cyclical_day":
					feature_space_definitions.update({column_name:keras.utils.FeatureSpace.feature(preprocessor=keras.layers.Lambda(cyclical_day,dtype=tf.float64), dtype=tf.float64,output_mode="float"),
													}) # minutes,hours, and days in datetime are encoded using cyclical features
				else:
					raise ValueError(f"{feature_spec['output_mode']} is not valid!")
...
def apply_featurespace(dataset:tf.data.Dataset,feature_space) -> tf.data.Dataset:
	...
	assert set(list(dataset.element_spec.keys())) == set(list(feature_space.features.keys())), f"Dataset features:{set(list(dataset.element_spec.keys()))} and feature space features:{set(list(feature_space.features.keys()))} should be same!"
	print(f"Applying feature space to raw dataset using map...")	
	#print(f"Element spec before applying featurespace:{dataset.element_spec}")
	...
	if columns_dataset == list(dataset.element_spec.keys()):
		...
		print(f"Dataset after feature space has different columns order:{list(dataset.element_spec.keys())} -- applying reorder_dataset method using map")
...
def get_train_test_eval_dataset(dataset:tf.data.Dataset,train_fraction:float=0.8,test_fraction:float=0.1)-> Tuple[tf.data.Dataset, tf.data.Dataset, tf.data.Dataset]:
	"""Apply the feature space on a tf dataset"""
...
def get_input_target_dataset(config_dict:dict,dataset,dataset_columns:list[str],
							 input_features_numerical:list[str],input_features_onehot:list[str],input_features_time:list[str],
							 target_features_numerical:list[str],target_features_onehot:list[str],target_features_time:list[str],						     
	...
	input_dataset,input_features_numerical_updated,input_features_onehot_expanded,input_features_time_updated = get_dataset_from_rawdataset(dataset,dataset_columns,"input",
	  														   input_features_numerical,input_features_onehot,input_features_time,feature_datetime,															   
														       plot_datasets=plot_datasets)
	input_features = get_combined_featurenames(input_features_numerical_updated,input_features_onehot_expanded,input_features_time_updated)
	...
	if add_input_normalizer:
		...
		if input_normalizer is None:
			input_normalizer = get_normalizer_from_tfdataset(input_dataset,input_features,n_elements_to_adapt_normalizer,skip_normalization=skip_normalization)
		else:
			print(f"Using normalizer with mean:{input_normalizer.mean.numpy()},var:{input_normalizer.variance.numpy()} provided in argument...")
			check_normalizer(input_normalizer,input_dataset)
		input_dataset = get_normalized_dataset(input_dataset,input_normalizer,"input",n_elements_to_plot=100,plot_datasets=False)
	else:
		input_normalizer = None
	input_dataset= get_transformed_dataset(input_dataset,window_size,batch_size,"input")
	
	target_dataset,target_features_numerical_updated,target_features_onehot_expanded,target_features_time_updated = get_dataset_from_rawdataset(dataset,dataset_columns,"target",
	...
	target_features = get_combined_featurenames(target_features_numerical_updated,target_features_onehot_expanded,target_features_time_updated)
...
def get_combined_featurenames(input_features_numerical:List[str]=[],input_features_onehot:List[str]=[],input_features_time:List[str]=[]):
	"""Get combined feature names in correct order"""

	return input_features_numerical + input_features_onehot + input_features_time

def get_final_dataset(dataset:tf.data.Dataset,features:List[str],featurespace,window_size:int,batch_size:int,dataset_name:str,data_type:str="float64"):
...
def cyclical_hour(x):
  ...
  return keras.ops.concatenate([keras.ops.sin(x*(2*3.14/(60*60))), keras.ops.cos(x*(2*3.14/(60*60)))], axis=-1)

def cyclical_day(x):
  """Custom layer to perform sine-cosine transformation."""
  
  return keras.ops.concatenate([keras.ops.sin(x*(2*3.14/(24*60*60))), keras.ops.cos(x*(2*3.14/(24*60*60)))], axis=-1)

def resample_csvdataset(dataset:tf.data.Dataset,column_datetime:str,time_interval_original:int=None,time_interval_desired:int=None,column_plot:str=None) -> tf.data.Dataset:

    n_original_data = 9000
    ...
    plt.plot(timestamps, values, label=f'Original Data (n={n_original_data})', marker='o', color='blue', linestyle='', markersize=6) # Plot original time series	
    plt.plot(timestamps_resampled, values_resampled, label=f'Resampled Data (n={n_resampled_data})', marker='x', color='red', linestyle='', markersize=8) # Plot resampled time series
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.show()
...
def add_select_resample_csvdataset(dataset:tf.data.Dataset,columns_added:List[str],columns_selected:List[str],column_datetime:str,column_datetimedict,time_interval_original:int=None,time_interval_desired:int=None,column_name:str=None,resample:bin=False) -> tf.data.Dataset:
    if columns_added:
